...
const PaymentServer = {
  ...
  create: async (data) => {
    try {
      const pagamento = await prisma.payment.create({ data })
  ...
  findOne: async (id) => {
    try {
      ...
      Logger('error', `Erro ao trazer info do pagamento ${id}: ${error.message}`)
      throw new Error(`Erro ao trazer info do pagamento ${id}: ${error.message}`)
    }
  },
  ...
  update: async (id, data) => {
    try {
      const pagamento = await prisma.payment.update({ where: { id }, data })
